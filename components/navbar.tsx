...
export default function Navbar({ token, userId, allGenres }: { token?: string | null, userId?: number | null, allGenres: Genre[] }) {
  ...
  const [genresTimeoutId, setGenresTimeoutId] = useState<NodeJS.Timeout | null>(null);
    ...
    <nav className="relative bg-[#000814] text-white">
      ...
      <div className={`${isNavOpen ? "flex" : "hidden"} bg-[#010f24] lg:hidden`}>
        <ul role="menu" className="flex flex-col px-5 gap-4 py-3">
          ...
          <li role="menuitem">
            <Link href="/movie" className="hover:bg-[#041b3d]">All Movies</Link>
          </li>
          <li role="menuitem" className="relative">
            <button onClick={genresHandler} className="hover:bg-[#041b3d] block p-2 rounded w-full text-left flex justify-between items-center">
              Genres
              <span className={`transform transition-transform duration-200 ${isGenresOpen ? 'rotate-180' : ''}`}>▼</span>
            </button>
            {isGenresOpen && (
              <ul className="mt-2 ml-4 space-y-1 bg-[#001225] rounded p-2">
                <li>
                  <Link href="/genres" className="hover:bg-[#041b3d] block p-2 rounded text-sm transition-colors" onClick={() => { setOpen(false); setGenresOpen(false); }}>All Genres</Link>
                </li>
                {allGenres.map((genre) => (
                  <li key={genre.id}>
                    <Link href={`/genres/${encodeURIComponent(genre.nombre)}`} className="hover:bg-[#041b3d] block p-2 rounded text-sm transition-colors" onClick={() => { setOpen(false); setGenresOpen(false); }}>
                      {genre.nombre}
                    </Link>
                  </li>
                ))}
      ...
      {!isAuthPage && (
        isLoggedIn ?
          <div className={`${isProfileOpen ? "flex" : "hidden"} grid grid-column bg-[#010f24] lg:hidden justify-end`}>
            <form action={logOut}>
              <button type="submit" className="hover:bg-[#041b3d] px-5 gap-4 py-3">Logout</button>
            </form>
            <button onClick={clicked} className="mb-2">Profile</button>
          </div>
          :
      ...
      <div className="hidden lg:grid grid-cols-3 items-center px-6 py-4 bg-[#000814] text-white shadow">
        <div className="flex gap-4">
          <Link href="/" className="hover:bg-[#041b3d] p-1 rounded">Home</Link>
          <Link href="/movie" className="hover:bg-[#041b3d] p-1 rounded">All Movies</Link>
          {/* Genres dropdown for desktop */}
          <div className="relative" onMouseEnter={handleGenresMouseEnter} onMouseLeave={handleGenresMouseLeave}>
            <Link href="/genres" className="hover:bg-[#041b3d] p-1 rounded flex items-center gap-1">
              Genres
              <span className={`text-xs transform transition-transform ${isGenresOpen ? 'rotate-180' : ''}`}>▼</span>
            </Link>
            ...
            {isGenresOpen && (
              <div className="absolute top-full left-0 bg-[#010f24] border border-[#041b3d] rounded-md shadow-lg z-50 min-w-48">
                <ul className="py-2">
                  <li>
                    <Link href="/genres" className="block px-4 py-2 hover:bg-[#041b3d] text-sm">
                      All Genres
                    </Link>
                  </li>
                  {allGenres.map((genre) => (
                    <li key={genre.id}>
                      <Link href={`/genres/${encodeURIComponent(genre.nombre)}`} className="block px-4 py-2 hover:bg-[#041b3d] text-sm">
                        {genre.nombre}
                      </Link>
                    </li>
                  ))}
        ...
        <div className="justify-self-end">
          {isLoggedIn ? (
            <div className="flex">
              <form action={logOut}>
                <button type="submit" className="hover:bg-[#041b3d] p-1 rounded mr-3">Logout</button>
              </form>
            ...
            !isAuthPage && (
              <div className="flex gap-4">
                <Link href="/login" className="hover:bg-[#041b3d] p-1 rounded">Log in</Link>
                <Link href="/signup" className="hover:bg-[#041b3d] p-1 rounded">Sign up</Link>
              </div>
            )
          )}
        </div>
