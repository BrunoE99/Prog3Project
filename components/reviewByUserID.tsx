...
export default async function ReviewsByUserID({ userId, pagination }: { userId: number, pagination: number }) {
    const reviewsUser = await reviewByUserID(userId, pagination);

    // console.log(reviewsUser.status);
    ...
    if (reviewsUser.status === 400) {
        return (
            <section className="px-6 py-10">
                ...
                <div className="mb-10 bg-[#001d3d] rounded-lg">
                    <div className="grid grid-cols-1 gap-6 p-5">
                        <h1 className="text-red-900 text-center text-2xl">{reviewsUser.message}</h1>
                    </div>
                    <Pagination currentPage={pagination} hasMovies={reviewsUser?.hasMovies ?? false}/>
                </div>
            </section>
    ...
    return (
        <section className="px-6 py-10">
            ...
            <div className="mb-10 bg-[#001d3d] rounded-lg">
                <div className="grid grid-cols-1 gap-6 p-5">
                    {reviewsUser.reviews.map((review: ReviewComponents, index: number) => (
