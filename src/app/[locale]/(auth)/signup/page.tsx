...
export default function Signup() {
  ...
  return (
    <form
      action={action}
      className="flex flex-col gap-3 items-center m-5 bg-white rounded-2xl shadow-md w-full md:w-100 py-20"
    >
      <div className="flex gap-2 items-center justify-center w-100">
        <input
          ...
          className="border rounded-md p-1 text-black bg-gray-100 w-[80%]"
        />
      </div>
      ...
      {state?.errors?.email && (
        <p className="text-red-500 text-center">{state.errors.email}</p>
      )}
      <div className="flex gap-2 items-center justify-center w-100">
        <input
          id="password"
      ...
      {state?.message && (
        <h1 className="border rounded p-2">{state.message}</h1>
      )}
